...
const PropertyCard = ({ listing, wishlist, toggleWishlist }) => {
  ...
  return (
    <>
      ...
      <div
        className="block md:hidden min-w-[70%] max-w-xs bg-white rounded-xl shadow-md p-3 flex-shrink-0 cursor-pointer mx-2"
        onClick={() => navigate(`/listings/${listing._id}`)}
      >
        ...
        <div className="flex justify-between text-xs text-gray-600 mt-1">
          ...
          <span className="flex items-center">
            <Star className="w-3 h-3 fill-[#ff385c] text-[#ff385c] mr-1" />
            {listing.rating || "New"}
          </span>
        </div>
      </div>

      {/* ✅ Full Card for Medium & Large Screens */}
      <div
        className="hidden md:block bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 cursor-pointer"
        onClick={() => navigate(`/listings/${listing._id}`)}
      >
        {/* Image Gallery */}
        <div className="relative h-64 overflow-hidden group">
